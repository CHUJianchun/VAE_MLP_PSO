...
def create_mlp_dataset(params, vae_file_name):
    try:
    ...
    for data_point in tqdm(data_points_list):
        ...
        data_point.encode = vae.get_mid(
            anion_variable.view(1, 1, anion_variable.shape[0], anion_variable.shape[1]),
            cation_variable.view(1, 1, cation_variable.shape[0], cation_variable.shape[1])).detach().cpu()
    ...
    def compose_t_p_encode(data_point_list):
        ...
        for data_point_ in data_point_list:
            input_data = torch.cat([data_point_.temperature.reshape(-1), data_point_.pressure.reshape(-1),
    ...
    viscosity_mlp_dataset = MlpDataset(viscosity_dataset_input, viscosity_dataset_output)
    thermal_conductivity_mlp_dataset = MlpDataset(thermal_conductivity_input, thermal_conductivity_output)
    heat_capacity_mlp_dataset = MlpDataset(heat_capacity_input, heat_capacity_output)
    ...
    torch.save(viscosity_mlp_dataset, 'Data/viscosity_mlp_dataset_' + vae_name + '.pkl')
    torch.save(viscosity_mlp_train_loader, 'Data/viscosity_mlp_train_loader_' + vae_name + '.pkl')
    torch.save(thermal_conductivity_mlp_dataset, 'Data/thermal_conductivity_mlp_dataset_' + vae_name + '.pkl')
    torch.save(thermal_conductivity_mlp_train_loader, 'Data/thermal_conductivity_mlp_train_loader_' + vae_name + '.pkl')
    torch.save(heat_capacity_mlp_dataset, 'Data/heat_capacity_mlp_dataset_' + vae_name + '.pkl')
    torch.save(heat_capacity_mlp_train_loader, 'Data/heat_capacity_mlp_train_loader_' + vae_name + '.pkl')
